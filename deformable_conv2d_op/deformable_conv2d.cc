...
REGISTER_OP("DeformableConv2DBackProp")
    ...
    .Doc(R"doc(only support NCHW now and only support GPU version)doc");

// this op is used to calculate the gradient of the deformable con2d
...
class DeformableConv2DBackPropOp : public OpKernel{
    ...
    void Compute(OpKernelContext* context) override{
        ...
        const Tensor& mask = context->input(3);
        const TensorShape& mask_shape = mask.shape();
        const Tensor& out_grad = context->input(4);
